...
def predict(query:str):
    ...
    top_pred = { "binding_score": preds[top_idx_val]}
    # top_pred = {"label": labels_legend_inverted[str(top_idx_val)],  "confidence": preds[top_idx_val]}
    labeled_preds = [{"binding_score": x} for x in enumerate(list(preds))]
    # labeled_preds = [{"label": labels_legend_inverted[str(i)], "confidence": x} for i, x in enumerate(list(preds))]
    return json.loads(json.dumps({"top": top_pred, "binding_score": labeled_preds}, cls=NumpyEncoder))
